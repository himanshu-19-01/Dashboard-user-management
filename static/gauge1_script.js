fetch("static//variables.json")
  ...
  .then(data => {
    ...
    console.log(data);
    var number =  data[0].Exec_Time_url1
    const myFloat = parseFloat(number);
    const value = myFloat.toFixed(2);
    //value=15
const myElement = document.getElementById("dot");
const myParagraph = document.getElementById("para");
if(data[0].Overall_Status=="Site down"){
  myElement.style.backgroundColor = "red";
  myParagraph.textContent = "Site down";
}
else{
  myElement.style.backgroundColor="green";
  myParagraph.textContent = "Site up";
}
if (value<=10)
{
  var data = {
